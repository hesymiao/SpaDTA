...
def _normalize_output_prefix_name(output_prefix_name: str) -> str:
    normalized = Path(output_prefix_name).name
    if normalized.endswith(".h5ad"):
        normalized = Path(normalized).stem
    return normalized


def _validate_sample_names(sample_names: list[str], sample_count: int) -> list[str]:
    cleaned = [str(sample_name).strip() for sample_name in sample_names if str(sample_name).strip()]
    if len(cleaned) != sample_count:
        raise ValueError(
            f"sample_count={sample_count} but received {len(cleaned)} non-empty sample names: {cleaned}"
        )
    if len(set(cleaned)) != len(cleaned):
        raise ValueError(f"Duplicate sample names are not allowed: {cleaned}")
    if sample_count < 2:
...
def build_horizontal_joint_adata(
    ...
    if input_h5ad_path is not None:
        ...
        unique_samples = sorted(joint_adata.obs[batch_key].astype(str).unique().tolist())
        if len(unique_samples) != sample_count:
            raise ValueError(
                f"sample_count={sample_count}, but obs[{batch_key!r}] has {len(unique_samples)} unique samples: "
                f"{unique_samples}"
            )
        return joint_adata, {
    ...
    if processed_root is None:
        raise ValueError("processed_root is required when input_h5ad_path is not provided.")
    if sample_names is None:
        raise ValueError("sample_names is required when input_h5ad_path is not provided.")

    processed_root = Path(processed_root).expanduser()
    if not processed_root.exists():
        raise FileNotFoundError(f"找不到 processed_root: {processed_root}")

    selected_samples = _validate_sample_names(sample_names, sample_count)
    ...
    for sample_name in selected_samples:
        input_h5ad = processed_root / f"{sample_name}.h5ad"
        if not input_h5ad.exists():
            raise FileNotFoundError(f"缺少样本文件: {input_h5ad}")
        ...
        if adata.var_names.has_duplicates:
            raise ValueError(f"{sample_name} has duplicate feature names.")
        ...
        for feature_name, feature_type in zip(adata.var["name"].astype(str), adata.var["type"].astype(str)):
            ...
            if previous_type is not None and previous_type != feature_type:
                raise ValueError(
                    f"Feature type mismatch for {feature_name}: {previous_type} vs {feature_type}"
                )
...
def train_horizontal_spatial_model(
    ...
    print(f"[train-horizontal] output_prefix={output_prefix_path}", flush=True)
    print(f"[train-horizontal] device={device}", flush=True)
    print(f"[train-horizontal] sample_count={sample_count}", flush=True)
    print(f"[train-horizontal] batch_key={batch_key}", flush=True)
    ...
    print("[train-horizontal] building joint adata", flush=True)
    ...
    print("[train-horizontal] building model", flush=True)
    model = DecAlignSpatialMetaLinearHorizontal(
        joint_adata,
        proj_dim=proj_dim,
        token_dim=token_dim,
    ...
    print("[train-horizontal] start fit", flush=True)
    loss_dict = model.fit(
        max_epoch=max_epoch,
    ...
    joint_adata.uns["horizontal_integration_decalign_linear"] = {
        ...
        "sample_count": int(sample_count),
        "horizontal_weight": float(horizontal_weight),
        "posthoc_batch_method": normalize_posthoc_batch_method(posthoc_batch_method),
        ...
        json.dumps(
            {
                ...
                "balance_start_epoch": balance_start_epoch,
                "balance_ema": balance_ema,
                "balance_weight_floor": balance_weight_floor,
                "spatial_coord_hidden_dim": spatial_coord_hidden_dim,
                "spatial_context_hidden_dim": spatial_context_hidden_dim,
                "spatial_context_k": spatial_context_k,
                "spatial_encoder_mode": spatial_encoder_mode,
                "spatial_fourier_scales": list(spatial_fourier_scales),
                "spatial_token_scale": spatial_token_scale,
                "spatial_token_dropout": spatial_token_dropout,
                "spatial_consistency_weight": spatial_consistency_weight,
                "spatial_consistency_warmup_epochs": spatial_consistency_warmup_epochs,
    ...
    print(f"[train-horizontal] output_h5ad={output_h5ad_path}", flush=True)
    print(f"[train-horizontal] loss_figure={loss_figure_path}", flush=True)
    print(f"[train-horizontal] param_json={param_json_path}", flush=True)
...
def run_horizontal_samples(
    ...
    print(f"[run-horizontal] config_name={config_name}", flush=True)
    print(f"[run-horizontal] output_root={output_root}", flush=True)
    print(f"[run-horizontal] output_prefix_name={output_prefix_name}", flush=True)
    if input_h5ad_path is not None:
        print(f"[run-horizontal] input_h5ad={Path(input_h5ad_path).expanduser()}", flush=True)
    if processed_root is not None:
        print(f"[run-horizontal] processed_root={Path(processed_root).expanduser()}", flush=True)
    if sample_names is not None:
        print(f"[run-horizontal] sample_names={sample_names}", flush=True)
