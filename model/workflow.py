...
def train_spatial_model(
    ...
    if max_cells != 0 or feature_input_mode:
        raise ValueError("Training-time subsetting and feature input conversion have been removed; preprocess the h5ad")
    ...
    shared_rep = contribution_details.get("shared_rep")
    q_mu_shared = contribution_details.get("q_mu_shared")

    q_mu_npy_path = output_prefix_path.parent / f"{output_prefix_path.name}_q_mu.npy"
    shared_rep_npy_path = output_prefix_path.parent / f"{output_prefix_path.name}_shared_rep.npy"
    q_mu_shared_npy_path = output_prefix_path.parent / f"{output_prefix_path.name}_q_mu_shared.npy"

    joint_adata.layers["reconstruction_decalign_linear"] = reconstruction
    joint_adata.obsm["X_emb_decalign_linear"] = latent_embedding
    joint_adata.obsm["X_q_mu_decalign_linear"] = latent_embedding
    ...
    np.save(q_mu_npy_path, latent_embedding)
    if shared_rep is not None:
        joint_adata.obsm["X_shared_rep_decalign_linear"] = shared_rep
        np.save(shared_rep_npy_path, shared_rep)
    if q_mu_shared is not None:
        joint_adata.obsm["X_q_mu_shared_decalign_linear"] = q_mu_shared
        np.save(q_mu_shared_npy_path, q_mu_shared)
    ...
    print(f"q_mu 已保存到: {q_mu_npy_path}")
    if shared_rep is not None:
        print(f"shared_rep 已保存到: {shared_rep_npy_path}")
    if q_mu_shared is not None:
        print(f"q_mu_shared 已保存到: {q_mu_shared_npy_path}")
...
def run_single_sample(
    ...
    gt_h5ad_path = None if gt_root is None else Path(gt_root) / f"adata_joint_{sample_name}_hvf2800.h5ad"
    ...
    if perform_evaluation:
        if gt_h5ad_path is None:
            raise ValueError("perform_evaluation=True 时必须提供 gt_root。")
