...
class DecAlignSpatialMetaLinearHorizontal(DecAlignSpatialMetaLinear):
    ...
    def _init_spatial_context(self, coords: np.ndarray) -> None:
        ...
        max_k = 0
        idx_chunks: list[tuple[np.ndarray, np.ndarray]] = []
        rel_chunks: list[tuple[np.ndarray, np.ndarray]] = []
        dist_chunks: list[tuple[np.ndarray, np.ndarray]] = []
        ...
        for label in pd.unique(labels):
            ...
            idx_chunks.append((global_idx, local_idx))
            rel_chunks.append((global_idx, local_rel))
            dist_chunks.append((global_idx, local_dist))
        ...
        if max_k > 0:
            ...
            for global_idx, local_idx in idx_chunks:
                k = local_idx.shape[1]
                if k:
                    neighbor_idx[global_idx, :k] = local_idx
            for global_idx, local_rel in rel_chunks:
                k = local_rel.shape[1]
                if k:
                    neighbor_rel[global_idx, :k, :] = local_rel
            for global_idx, local_dist in dist_chunks:
    ...
    def _fetch_batch_codes(self, indices: np.ndarray) -> Optional[torch.Tensor]:
        ...
        return torch.hstack(code_tensors)
    ...
    def decode(
        ...
        hidden_st = self.decoder_st(torch.cat([H["z_shared"], z_st_for_decoder, batch_context], dim=1))
        hidden_sm = self.decoder_sm(torch.cat([H["z_shared"], z_sm_for_decoder, batch_context], dim=1))
