...
class DecAlignSpatialMetaLinear(BaseDecAlignSpatialMetaLinear):
    ...
    @torch.no_grad()
    def generate_source_only_with_indices(
        self,
        X: torch.Tensor,
        indices: np.ndarray,
        source_modality: str,
        ...
        """Generate from one modality without using the target private branch."""
        source = str(source_modality).strip().lower()
        if source not in {"st", "sm"}:
            raise ValueError("source_modality must be 'st' or 'sm'.")
        H = self.encode_with_indices(X, indices=indices)
        source_common = H["homo_st_embedding"] if source == "st" else H["homo_sm_embedding"]
        zero_common = torch.zeros_like(source_common)
        shared_rep = self.shared_resample(torch.cat([source_common, zero_common], dim=1))
        z_shared = self.q_mu_shared_fc(shared_rep)
        zero_private_st = torch.zeros(
            (X.shape[0], self.n_latent_st), dtype=z_shared.dtype, device=z_shared.device
        )
        zero_private_sm = torch.zeros(
            (X.shape[0], self.n_latent_sm), dtype=z_shared.dtype, device=z_shared.device
        )
        generation_latent = {
            "z_shared": z_shared,
            "z_st": zero_private_st,
            "z_sm": zero_private_sm,
        }
        output = self.decode(generation_latent, lib_size)
        output["source_only_shared_embedding"] = shared_rep
        output["source_only_z_shared"] = z_shared
        return output
