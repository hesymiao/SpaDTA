...
def log_stage(message: str) -> None:
    print(f"[fig2k] {message}", flush=True)
...
def plot_spot_pcc_on_top_third(
    ...
    y_true = to_dense_float32(eval_adata.layers["true_st_log1p_for_eval"])
    y_pred = to_dense_float32(eval_adata.layers["generated_st_log1p_sm_only"])
    ...
    figure_base = f"{sample_name}_sm_to_st_spot_pcc_top_third"
    ...
    summary_path = figure_dir / f"{sample_name}_fig2k_summary.json"
...
def run_sm_to_st_generation_spatial_top_third() -> dict[str, object]:
    ...
    log_stage(f"mean train ST library size for inference={fixed_st_libsize:.6f}")
    ...
    log_stage("generating ST from held-out SM with fixed train-mean library size")
    ...
    feature_df, metrics = evaluate_generation(generation, st_feature_names)
    ...
    eval_h5ad_path = experiment_dir / "eval_generated_st_from_sm_only.h5ad"
...
if __name__ == "__main__":
    run_sm_to_st_generation_spatial_top_third()
