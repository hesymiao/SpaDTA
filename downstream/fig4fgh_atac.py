...
OUT_DIR = PROJECT_ROOT / "SpaDTA_718/runs/atac_downstream/fig4fgh_atac"
...
def load_method_order(metrics_dir: Path) -> list[str]:
    table = pd.read_csv(metrics_dir / "ARI.csv")
...
def prepare_metrics_table(metrics_dir: Path) -> pd.DataFrame:
    ...
    for metric in METRICS:
        wide = pd.read_csv(metrics_dir / f"{metric}.csv").drop(columns="mean", errors="ignore")
        frames.append(wide.melt(id_vars="method", var_name="sample", value_name=metric))
    return frames[0].merge(frames[1], on=["method", "sample"], validate="one_to_one")
...
def plot_group_metric(
    ...
    fig.tight_layout()
    out_png = out_dir / f"{group_name}_{metric}_mean_bar.png"
    out_pdf = out_dir / f"{group_name}_{metric}_mean_bar.pdf"
    out_svg = out_dir / f"{group_name}_{metric}_mean_bar.svg"
    fig.savefig(out_png, bbox_inches="tight")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot Fig. 1 F-H ARI/NMI bars by dataset group.")
